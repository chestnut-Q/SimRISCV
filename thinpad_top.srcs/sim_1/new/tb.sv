...
module tb;
  ...
  reg push_btn;   // BTN5 ????????????????﹞???????? 1
  reg reset_btn;  // BTN6 ??竹?????????????﹞???????? 1

  reg [3:0] touch_btn; // BTN1~BTN4???????????????? 1
  reg [31:0] dip_sw;   // 32 竹?????????????ON???? 1

  wire [15:0] leds;  // 16 竹 LED?????? 1 ????
  wire [7:0] dpy0;   // ??????竹????????妊??????? 1 ????
  wire [7:0] dpy1;   // ??????竹????????妊??????? 1 ????
  ...
  wire [31:0] base_ram_data;  // BaseRAM ??????? 8 竹?? CPLD ?????????????
  wire [19:0] base_ram_addr;  // BaseRAM ???
  wire[3:0] base_ram_be_n;    // BaseRAM ???????????完?????????????????????? 0
  wire base_ram_ce_n;  // BaseRAM ????????完
  wire base_ram_oe_n;  // BaseRAM ??????????完
  wire base_ram_we_n;  // BaseRAM 忱????????完

  wire [31:0] ext_ram_data;  // ExtRAM ????
  wire [19:0] ext_ram_addr;  // ExtRAM ???
  wire[3:0] ext_ram_be_n;    // ExtRAM ???????????完?????????????????????? 0
  wire ext_ram_ce_n;  // ExtRAM ????????完
  wire ext_ram_oe_n;  // ExtRAM ??????????完
  wire ext_ram_we_n;  // ExtRAM 忱????????完

  wire [22:0] flash_a;  // Flash ?????a0 ???? 8bit ????完??16bit ????????
  wire [15:0] flash_d;  // Flash ????
  wire flash_rp_n;   // Flash ??竹????????完
  wire flash_vpen;   // Flash 忱????????????????????????忱
  wire flash_ce_n;   // Flash ??????????完
  wire flash_oe_n;   // Flash ?????????????完
  wire flash_we_n;   // Flash 忱???????????完
  wire flash_byte_n; // Flash 8bit ????????完??????? flash ?? 16 竹???????? 1
